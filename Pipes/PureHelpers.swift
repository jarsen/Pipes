...
public func map<S: Sequence, T>(_ f: (S.Iterator.Element)->T, _ seq: S) -> [T] {
    return map(seq as! (_) -> _, f)
}

public func filter<S: Sequence>(_ f: (S.Iterator.Element)->Bool, _ seq: S) -> [S.Iterator.Element] {
    return filter(seq as! (_) -> Bool, f)
}

public func reduce<S : Sequence, U>(_ initial: U, f: (U, S.Iterator.Element) -> U, _ seq: S) -> U {
...
public func extend<S: Sequence, C: ExtensibleCollectionType>(_ collection: C, newElements: S) -> C where S.Iterator.Element == C.Generator.Element {
    var collection = collection
    collection.extend(newElements)
    return collection
}

public func extend<S: Sequence, C: ExtensibleCollectionType>(_ newElements: S, _ collection: C) -> C where S.Iterator.Element == C.Generator.Element {
...
public func replaceRange<R: RangeReplaceableCollection, C : Collection>(_ collection: R, subRange: Range<R.Index>, with newElements: C) -> R where C.Iterator.Element == R.Iterator.Element {
    var collection = collection
    collection.replaceSubrange(subRange, with: newElements)
    return collection
}

public func replaceRange<R: RangeReplaceableCollection, C : Collection>(_ subRange: Range<R.Index>, with newElements: C, _ collection: R) -> R where C.Iterator.Element == R.Iterator.Element {
...
public func splice<R: RangeReplaceableCollection, S : Collection>(_ collection: R, newElements: S, atIndex i: R.Index) -> R where S.Iterator.Element == R.Iterator.Element {
    var collection = collection
    collection.splice(newElements, atIndex: i)
    return collection
}

public func splice<R: RangeReplaceableCollection, S : Collection>(_ newElements: S, atIndex i: R.Index, _ collection: R) -> R where S.Iterator.Element == R.Iterator.Element {
...
public func removeRange<R: RangeReplaceableCollection>(_ subRange: Range<R.Index>, _ collection: R) -> R {
    return removeRange(collection as! Range<_>, subRange)
}
...
public func removeValueForKey<T,U>(_ key: T, _ dictionary: [T:U]) -> ([T:U], removed: U?) {
    return removeValueForKey(dictionary, key as! [_ : _])
}
